    ...
    <taon-admin-mode-configuration>
      @if (itemsLoaded()) {
        ...
        <footer
          ...
          {{ t.gettext('Copyright') }} <strong>analytics</strong>
          {{ year }}
        </footer>
      }
    </taon-admin-mode-configuration>
...
export class AnalyticsApp implements OnInit {
  t = t.for(this);
  ...
  navItems =
    AnalyticsClientRoutes.length <= 1
      ? []
      : AnalyticsClientRoutes.filter(r => r.path !== undefined).map(r => ({
          path: r.path === '' ? '/' : `/${r.path}`,
...
//#region @browser
export const AnalyticsServerRoutes: ServerRoute[] = [
  {
    path: '**',
    renderMode: RenderMode.Prerender,
  },
];
export const AnalyticsClientRoutes: Routes = [
  {
    path: '',
    pathMatch: 'full',
    redirectTo: () => {
      if (AnalyticsClientRoutes.length === 1) {
        return '';
      }
      return AnalyticsClientRoutes.find(r => r.path !== '')!.path!;
    },
  },
  // PUT ALL ROUTES HERE
...
//#region  analytics app configs
//#region @browser
export const AnalyticsAppConfig: ApplicationConfig = {
  providers: [
    provideZonelessChangeDetection(),
    {
      provide: TAON_CONTEXT,
      useFactory: () => AnalyticsContext,
    },
    providePrimeNG({
      theme: {
        preset: Aura,
    ...
    }),
    {
      ...
      multi: true,
      useFactory: () => AnalyticsStartFunction,
    },
    provideBrowserGlobalErrorListeners(),
    // remove withHashLocation() to use SSR
    provideRouter(
      AnalyticsClientRoutes,
      withHashLocation(),
      withComponentInputBinding(),
    ),
...
export const AnalyticsServerConfig: ApplicationConfig = {
  providers: [provideServerRendering(withRoutes(AnalyticsServerRoutes))],
};

export const AnalyticsConfig = mergeApplicationConfig(
  AnalyticsAppConfig,
...
var AnalyticsContext = Taon.createContext(() => ({
  ...HOST_CONFIG['AnalyticsContext'],
...
//#region  analytics start function
export const AnalyticsStartFunction = async (
  ...
  await AnalyticsContext.initialize(startParams);
...
export default AnalyticsStartFunction;
//#endregion
